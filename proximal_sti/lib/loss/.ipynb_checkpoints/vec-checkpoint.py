...
class VEC_loss(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.cuda_device = device

    def forward(self, input_data, target, mask, gt_mean, gt_std, ani_mask, thr=0):
...
#         print(new_input.shape, mask.shape, ani_mask.shape)
        bool_mask = ani_mask > thr
#         bool_mask = mask[:, 0, 0, :, :, :] == 1
        _, input_eigvec = torch.symeig(new_input[bool_mask], True)
        _, target_eigvec = torch.symeig(new_target[bool_mask], True)
