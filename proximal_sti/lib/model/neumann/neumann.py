...
class NEUMANN(nn.Module):
	...
	def forward(self, y, dk, mask, ls100):
		...
		out = torch.zeros_like(x_est)
		out += x_est

		for i in range(self.iter_num):
			
			if i == 0:
				beta = torch.zeros_like(x_est)
				beta += x_est
			
			grad_branch = beta - self.alpha * torch.sum(torch.ifft(dk*torch.rfft(torch.ifft(torch.sum(dk*torch.rfft(F.pad(beta, (0, pad_z, 0, pad_y, 0, pad_x)), 3, normalized=True, onesided=False).unsqueeze(2), 1, keepdim=True), 3, normalized=True)[:, :, :, :, :, :, 0]* pad_mask, 3, normalized=True, onesided=False), 3, normalized=True)[:, :, :, :x_dim, :y_dim, :z_dim, 0], 2)
	

			#x_input = ((beta - mean) / std) * mask[:, :, 0, :, :, :]
			#x_res = self.gen(x_input)
			#reg_branch = -1 * self.alpha * (((x_res * std) + mean) * mask[:, :, 0, :, :, :])
			
			reg_branch = -1 * self.alpha * self.gen(beta)

			beta = (grad_branch + reg_branch) * mask[:, :, 0, :, :, :]
			out += beta

		out = ((out - mean) / std) * mask[:, :, 0, :, :, :]
